...
try
    if colocalization
        if ~contrast_flag
            data12CH(:,:,1) = imadjust(data12CH(:,:,1));
            data12CH(:,:,2) = imadjust(data12CH(:,:,2));
            disp("Automatic contrast limits applied for analysis in COLOCALIZATION add-on.");
        elseif contrast_flag                         
            T = graythresh(data12CH(:,:,1));   
            lh_lim_coloc_temp = [T*0.2, T];             
            data12CH(:,:,1) = imadjust(data12CH(:,:,1), lh_lim_coloc_temp);                          
            ...
            data12CH(:,:,1) = imadjust(data12CH(:,:,1), lh_lim_coloc(1:2));
            data12CH(:,:,2) = imadjust(data12CH(:,:,2), lh_lim_coloc(3:4));
            disp("User-chosen contrast limits applied for analysis in COLOCALIZATION add-on.");
        end
        
        if ~contrast_visual_flag
            disp_im_coloc1 = imadjust(data12CH(:,:,1), lh_lim_coloc_user(1:2));
            disp_im_coloc2 = imadjust(data12CH(:,:,2), lh_lim_coloc_user(3:4));
